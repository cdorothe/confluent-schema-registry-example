...
public class SchemaRegistry {
	...
	private static RestService restService = new RestService(
			new ArrayList<String>(Arrays.asList(com.chdor.schema_registry.example.Config.SCHEMA_REGISTRY_URL)));
...
//			testCompatibility(schemaString, schemaType, subject, restService);
	...
	public static void init() throws IOException, RestClientException {
		...
		// If subject parameter is null then the global compatibility is returned, else
		// the subject compatibility is returned.
		// Note that if no compatibility was set to the subject (default), it raises an
		// exception.
...
//			List<String> compatibilities = restService.testCompatibility(registerSchemaRequest, subject, String.valueOf(lastVersion), true);
	 ...
	 * 
	 * @param subjects : List of Subjects name
	 * @throws IOException
	 * @throws RestClientException
	 */
