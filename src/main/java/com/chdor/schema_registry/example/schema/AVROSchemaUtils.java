...
public class AVROSchemaUtils {
	...
	/**
	 * Update a Schema: Change a Field type and add two new Fields with an optional
	 * one
	 */
	public static void updateSchema() {
		...
		for (Schema.Field field : listOfSchemaFields) {
			...
			// change the "SérieTV" field: type: string->int. Add a doc to explain why its
			// changed
			// Add a note to explain why
			if (field.name().equals("tvShow")) {
				newField = new Schema.Field(field.name(), schemaType,
						"Type changed: string->int. It's now an index to the TVSeries table");
			} else {
		...
		// Note: We can't modify the original one because when a schema is created, it
		// is locked
		Schema schemav2 = Schema.createRecord("TVSeriesActor", "TV 1970-1980 Series Actors - V2",
				"com.chdor.schema_registry.example.avro.model", false);

		// Create an optional string field: synopsis
		// It's a union of schema.null and schema.type
		ArrayList<Schema> optionalString = new ArrayList<Schema>(
		...
		// Create an optional int field (it's a union of schema.type and schema.null):
		// Age
		newField = new Schema.Field("IsAlive", Schema.create(Schema.Type.BOOLEAN),
				"Indicate if the actor is alive or died", true);
		listOfNewFields.add(newField);
	...
	/**
	 * Load an AVRO Schema Use Confluent AvroSchemaProvider
	 */
	...
	public static void generateSchemaFromPOJOWithConfluent() {
		TVSeriesActorAVROPojo tvSeriesActorAVROPojo = new TVSeriesActorAVROPojo().setFirstName("Jaimie")
				.setLastName("Summers");
	...
	/**
	 * For Just Do It Retrieve the AVRO Schema from the generated classes
	 */
